import Characters.*;
import Fight.*;
import weapons.*;
import World.*;



public class Main {
    public static void main(String[] args) {
        Start start = new Start();

        Hero alex = new Hero("alex", "barbarian", listOfCommonWeapons.commonWeapons()[1]);
        Enemy fighter = new Enemy("malchuar", "vampire", 1, listOfCommonWeapons.commonWeapons()[1], true);
alex.setCurrHP(10);
        new fight(alex, fighter);
 Enemy enemy = new Enemy("minotaur", "barbarian", 1, listOfCommonWeapons.commonWeapons()[1], false);

new fight(alex, enemy);

        System.out.println("this should show up after the fight. ");
        new fight(alex, new Enemy("wolf", "assassin",1,listOfCommonWeapons.commonWeapons()[1],false));
    }


    }